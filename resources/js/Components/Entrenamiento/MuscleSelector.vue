...
// Mapeo de IDs del SVG a slugs de grupos musculares
const svgMuscleMap = {
    // Primera vista SVG
    chest: 'pecho',
    ...
    calves: 'pantorrillas',
    traps: 'trapecio',
    'traps-middle': 'trapecio',
    lats: 'espalda',
    'lowerback': 'baja-espalda',
    glutes: 'gluteos',
    hands: 'antebrazos',

    // Segunda vista SVG (con sufijo -b)
    'calves-b': 'pantorrillas',
    'quads-b': 'cuadriceps',
    'abdominals-b': 'abdomen',
    'obliques-b': 'oblicuos',
    'hands-b': 'antebrazos',
    'forearms-b': 'antebrazos',
    'biceps-b': 'biceps',
    'front-shoulders-b': 'hombros-frontales',
    'chest-b': 'pecho',
    'traps-b': 'trapecio'
};

const getMuscleColor = (slug) => muscleColors[slug] || '#6b7280';

const handleSVGClick = (muscleSlug) => {
    const group = props.muscleGroups.find(g => g.slug === muscleSlug);
    if (group) {
        const currentSelection = [...selectedGroups.value];
        const index = currentSelection.indexOf(group.id);
        if (index > -1) {
            currentSelection.splice(index, 1);
        } else {
            currentSelection.push(group.id);
        }
        selectedGroups.value = currentSelection;
    }
};

const isMusclePath = (id) => {
    const slug = svgMuscleMap[id];
    if (!slug) return false;
    const group = props.muscleGroups.find(g => g.slug === slug);
...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-3">
            ...
            <button type="button" @click="clearSelection" :disabled="selectedGroups.length === 0"
                class="px-3 py-1.5 text-xs rounded-md border border-slate-600 text-slate-300 hover:text-white hover:border-slate-500 disabled:opacity-40 disabled:cursor-not-allowed transition">
                Limpiar selección
            </button>
        </div>

        <!-- Vista interactiva del cuerpo -->
        <div class="bg-slate-800 rounded-lg p-6 flex gap-4 justify-center flex-wrap">
            <!-- Primera vista SVG -->
            <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg"
                class="w-full max-w-xs h-auto" style="max-height: 600px;">
                <defs>
                ...
                <g id="traps" class="bodymap muscle-path" @click="handleSVGClick('trapecio')"
                    :class="{ active: isMusclePath('traps'), inactive: !isMusclePath('traps') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.trapecio }">
                    <path
                ...
                <g id="calves" class="bodymap muscle-path" @click="handleSVGClick('pantorrillas')"
                    :class="{ active: isMusclePath('calves'), inactive: !isMusclePath('calves') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.pantorrillas }">
                    <path
                ...
                <g id="hamstrings" class="bodymap muscle-path" @click="handleSVGClick('isquiotibiales')"
                    :class="{ active: isMusclePath('hamstrings'), inactive: !isMusclePath('hamstrings') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.isquiotibiales }">
                    <path
                ...
                <g id="glutes" class="bodymap muscle-path" @click="handleSVGClick('gluteos')"
                    :class="{ active: isMusclePath('glutes'), inactive: !isMusclePath('glutes') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.gluteos }">
                    <path
                ...
                <g id="hands" class="bodymap muscle-path" @click="handleSVGClick('antebrazos')"
                    :class="{ active: isMusclePath('hands'), inactive: !isMusclePath('hands') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.antebrazos }">
                    <path
                ...
                <g id="forearms" class="bodymap muscle-path" @click="handleSVGClick('antebrazos')"
                    :class="{ active: isMusclePath('forearms'), inactive: !isMusclePath('forearms') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.antebrazos }">
                    <path
                ...
                <g id="triceps" class="bodymap muscle-path" @click="handleSVGClick('triceps')"
                    :class="{ active: isMusclePath('triceps'), inactive: !isMusclePath('triceps') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.triceps }">
                    <path
                ...
                <g id="lats" class="bodymap muscle-path" @click="handleSVGClick('espalda')"
                    :class="{ active: isMusclePath('lats'), inactive: !isMusclePath('lats') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.espalda }">
                    <path
                ...
                <g id="lowerback" class="bodymap muscle-path" @click="handleSVGClick('baja-espalda')"
                    :class="{ active: isMusclePath('lowerback'), inactive: !isMusclePath('lowerback') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors['baja-espalda'] }">
                    <path
                ...
                <g id="traps-middle" class="bodymap muscle-path" @click="handleSVGClick('trapecio')"
                    :class="{ active: isMusclePath('traps-middle'), inactive: !isMusclePath('traps-middle') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.trapecio }">
                    <path
                ...
                <g id="rear-shoulders" class="bodymap muscle-path" @click="handleSVGClick('hombros-traseros')"
                    :class="{ active: isMusclePath('rear-shoulders'), inactive: !isMusclePath('rear-shoulders') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors['hombros-traseros'] }">
                    <path
            ...
            <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg"
                class="w-full max-w-xs h-auto" style="max-height: 600px;">
                <defs>
                    <radialGradient id="jointradial2" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
                        <stop offset="0%" style="stop-color: rgb(254, 91, 127); stop-opacity: 1;"></stop>
                        <stop offset="100%" style="stop-color: rgb(231, 236, 239); stop-opacity: 1;"></stop>
                    </radialGradient>
                </defs>
                <g id="calves-b" class="bodymap muscle-path" @click="handleSVGClick('pantorrillas')"
                    :class="{ active: isMusclePath('calves-b'), inactive: !isMusclePath('calves-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.pantorrillas }">
                    <path
                ...
                <g id="quads-b" class="bodymap muscle-path" @click="handleSVGClick('cuadriceps')"
                    :class="{ active: isMusclePath('quads-b'), inactive: !isMusclePath('quads-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.cuadriceps }">
                    <path
                ...
                <g id="abdominals-b" class="bodymap muscle-path" @click="handleSVGClick('abdomen')"
                    :class="{ active: isMusclePath('abdominals-b'), inactive: !isMusclePath('abdominals-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.abdomen }">
                    <path
                ...
                <g id="obliques-b" class="bodymap muscle-path" @click="handleSVGClick('oblicuos')"
                    :class="{ active: isMusclePath('obliques-b'), inactive: !isMusclePath('obliques-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.oblicuos }">
                    <path
                ...
                <g id="hands-b" class="bodymap muscle-path" @click="handleSVGClick('antebrazos')"
                    :class="{ active: isMusclePath('hands-b'), inactive: !isMusclePath('hands-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.antebrazos }">
                    <path
                ...
                <g id="forearms-b" class="bodymap muscle-path" @click="handleSVGClick('antebrazos')"
                    :class="{ active: isMusclePath('forearms-b'), inactive: !isMusclePath('forearms-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.antebrazos }">
                    <path
                ...
                <g id="biceps-b" class="bodymap muscle-path" @click="handleSVGClick('biceps')"
                    :class="{ active: isMusclePath('biceps-b'), inactive: !isMusclePath('biceps-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.biceps }">
                    <path
                        d="M535.68,414.61c-8.57,4.33-23.53,1.71-39.79-18.59-.02-.02-.03-.04-.05-.06-9.71-13.16-19.05-25.49-21.34-27.65-.5-.47-1.05-.99-1.66-1.54-4.5-4.14-11.31-10.39-15.31-20.4-.14-.35-.37-.62-.67-.8,0,0,0,0,0,0-.86-2.84-1.54-5.75-2.01-8.66-1.1-6.96-4.03-25.45,5.81-33.63,5.53-4.61,13.2-4.37,21.33-4.14.92.03,2.27.1,3.94.29,2.54.31,5.82.88,9.54,1.98,2.49.75,5.18,1.73,7.97,3.01,3.26,1.5,6.39,3.29,9.32,5.29.45.54.96,1.08,1.5,1.67,2.81,3.02,7.61,8.17,16.01,24.38,25.45,57.59,13.79,73.98,4.57,78.62Z"
                        fill="currentColor"></path>
                    <path
                ...
                <g id="front-shoulders-b" class="bodymap muscle-path" @click="handleSVGClick('hombros-frontales')"
                    :class="{ active: isMusclePath('front-shoulders-b'), inactive: !isMusclePath('front-shoulders-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors['hombros-frontales'] }">
                    <path
                ...
                <g id="chest-b" class="bodymap muscle-path" @click="handleSVGClick('pecho')"
                    :class="{ active: isMusclePath('chest-b'), inactive: !isMusclePath('chest-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.pecho }">
                    <path
                ...
                <g id="traps-b" class="bodymap muscle-path" @click="handleSVGClick('trapecio')"
                    :class="{ active: isMusclePath('traps-b'), inactive: !isMusclePath('traps-b') && selectedGroups.length > 0 }"
                    :style="{ color: muscleColors.trapecio }">
                    <path
        ...
        <!-- Grid de grupos musculares para referencia rápida -->
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
            <button v-for="group in props.muscleGroups" :key="group.id" type="button"
                @click="toggleGroupSelection(group.id)"
                class="px-3 py-2 rounded-lg font-medium transition-all duration-200 text-white text-xs" :style="{
                    backgroundColor: getMuscleColor(group.slug),
                    opacity: selectedGroups.includes(group.id) ? 1 : 0.5,
                    scale: selectedGroups.includes(group.id) ? 1.05 : 1,
                }" :class="{
                    'ring-2 ring-blue-400': selectedGroups.includes(group.id)
                }">
                {{ group.nombre }}
            </button>
        </div>
        ...
        <!-- Resultado -->
        <div v-if="selectedGroups.length > 0" class="p-4 bg-emerald-950/50 border border-emerald-700 rounded-lg">
            <p class="text-emerald-300 text-sm font-medium mb-1">
                ✓ Grupos musculares seleccionados ({{ selectedGroups.length }})
            </p>
            <div class="space-y-2">
                <div v-for="groupId in selectedGroups" :key="groupId"
                    class="p-2 bg-emerald-900/30 rounded border border-emerald-600 flex items-center gap-3">
                    <div :style="{ width: '24px', height: '24px', backgroundColor: getMuscleColor(props.muscleGroups.find(g => g.id === groupId)?.slug), borderRadius: '4px' }"
                        class="flex-shrink-0"></div>
                    <div class="flex-1">
                        <p class="text-emerald-100 font-bold">
                            {{props.muscleGroups.find(g => g.id === groupId)?.nombre}}
                        </p>
                        <p class="text-emerald-400 text-xs">
                            {{props.muscleGroups.find(g => g.id === groupId)?.descripcion}}
                        </p>
                    </div>
                </div>
            </div>
